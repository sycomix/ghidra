...
def processDomainFile(domainFile):
    if (
        ...
        != domainFile.getContentType()
    ):
        return  # skip non-Program files
    if domainFile.isVersioned() and not domainFile.isCheckedOut():
        println(
            f"WARNING! Skipping versioned file - not checked-out: {domainFile.getPathname()}"
        )
    program = None
    ...
    try:
        ...
        println(
            f"ERROR! Failed to process file due to upgrade issue: {domainFile.getPathname()}"
        )
...
def processProgram(program):
    ...
    println(f"Processing: {program.getDomainFile().getPathname()}")
    monitor.setMessage(f"Processing: {program.getDomainFile().getName()}")
    ...
    try:
        ...
        printerr(
            f"ERROR! Exception occurred while processing file: {program.getDomainFile().getPathname()}"
        )
        printerr(f"       {Exception.getMessage()}")
        e.printStackTrace()
    ...
    # ...save any changes
    program.save(f"Changes made by script: {SUBSCRIPT_NAME}", monitor)
